...
func TestProcessRegistrySessionProvider_New(t *testing.T) {
	...
	provider := NewProcessRegistrySessionProvider(registry, aiClient, aiClient, aiClient, "/work/dir", 8765)
...
func TestProcessRegistrySessionProvider_GetProcessSessionID(t *testing.T) {
	registry := process.NewProcessRegistry()
	provider := NewProcessRegistrySessionProvider(registry, nil, nil, nil, "/work/dir", 8765)
...
func TestProcessRegistrySessionProvider_GenerateProcessMCPConfig_Worker_Amp(t *testing.T) {
	...
	provider := NewProcessRegistrySessionProvider(registry, aiClient, aiClient, aiClient, "/work/dir", 9999)
...
func TestProcessRegistrySessionProvider_GenerateProcessMCPConfig_Observer_HTTP(t *testing.T) {
	registry := process.NewProcessRegistry()
	aiClient := &mockHeadlessClient{clientType: client.ClientClaude}
	provider := NewProcessRegistrySessionProvider(registry, aiClient, aiClient, aiClient, "/work/dir", 8765)

	// Use the well-known observer ID
	config, err := provider.GenerateProcessMCPConfig(repository.ObserverID)
	require.NoError(t, err)
	require.Contains(t, config, "http://localhost:8765/observer")
	require.Contains(t, config, "perles-observer") // Observer server name
	require.Contains(t, config, "mcpServers")      // Claude format includes mcpServers wrapper
	// Ensure it's NOT pointing to worker endpoint (this was the bug!)
	require.NotContains(t, config, "/worker/")
}

func TestProcessRegistrySessionProvider_GenerateProcessMCPConfig_Observer_Amp(t *testing.T) {
	registry := process.NewProcessRegistry()
	aiClient := &mockHeadlessClient{clientType: client.ClientAmp}
	provider := NewProcessRegistrySessionProvider(registry, aiClient, aiClient, aiClient, "/work/dir", 9999)
	...
	config, err := provider.GenerateProcessMCPConfig(repository.ObserverID)
	require.NoError(t, err)
	require.Contains(t, config, "http://localhost:9999/observer")
	require.Contains(t, config, "perles-observer")
	// Amp format doesn't have mcpServers wrapper
	require.NotContains(t, config, "mcpServers")
	// Ensure it's NOT pointing to worker endpoint
	require.NotContains(t, config, "/worker/")
}
...
func TestProcessRegistrySessionProvider_GetWorkDir(t *testing.T) {
	registry := process.NewProcessRegistry()
	provider := NewProcessRegistrySessionProvider(registry, nil, nil, nil, "/my/work/dir", 8765)
...
func TestProcessRegistrySessionProvider_GetWorkDir_Empty(t *testing.T) {
	registry := process.NewProcessRegistry()
	provider := NewProcessRegistrySessionProvider(registry, nil, nil, nil, "", 8765)
...
func TestProcessRegistrySessionProvider_ImplementsSessionProvider(t *testing.T) {
	registry := process.NewProcessRegistry()
	provider := NewProcessRegistrySessionProvider(registry, nil, nil, nil, "/work", 8765)
...
func TestProcessRegistrySessionProvider_SessionCorrectlyWrapsProcess(t *testing.T) {
	registry := process.NewProcessRegistry()
	aiClient := &mockHeadlessClient{clientType: client.ClientClaude}
	provider := NewProcessRegistrySessionProvider(registry, aiClient, aiClient, aiClient, "/project", 8765)
