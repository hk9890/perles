...
var issueSectionRegex = regexp.MustCompile(`(?m)^## (Title|Description|Notes|Labels|Type|Status|Priority)\s*$`)
...
## Priority
P%d
`, strings.TrimSpace(issue.TitleText), issue.DescriptionText, issue.Notes, labelsBlock, issue.Type, issue.Status, issue.Priority)
}
...
func ParseIssueMarkdown(content string) (IssueEdit, error) {
	...
	required := []string{"Title", "Description", "Notes", "Labels", "Type", "Status", "Priority"}
	...
	issueType, err := parseType(sections["Type"])
	if err != nil {
		return IssueEdit{}, err
	}
...
func parseStatus(raw string) (beads.Status, error) {
	...
	switch status {
	case beads.StatusOpen, beads.StatusInProgress, beads.StatusClosed, beads.StatusDeferred, beads.StatusBlocked, beads.StatusPinned, beads.StatusHooked:
		return status, nil
	default:
		if status == "" {
			return "", fmt.Errorf("invalid status %q", strings.TrimSpace(raw))
		}
		return status, nil
	}
}

func parseType(raw string) (beads.IssueType, error) {
	issueType := beads.IssueType(strings.ToLower(strings.TrimSpace(raw)))
	if issueType == "" {
		return "", fmt.Errorf("invalid type %q", strings.TrimSpace(raw))
	}
